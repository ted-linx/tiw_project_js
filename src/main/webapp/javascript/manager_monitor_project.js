...
(() => {
    ...
    async function loadProjects() {
        try {
            const data = await getJSON(`${ctx}/manager-home?action=monitorProjectsList`);
    ...
    async function selectProject(projectId) {
        try {
            ...
            renderNav();
            const data = await getJSON(`${ctx}/manager-home?action=monitorProject&project_id=${encodeURIComponent(projectId)}`);
            renderDetails({
                ...data,
            ...
            showError("Error while loading project details.")
        }
    }
    ...
    function renderDetails(data) {
        ...
        details.innerHTML = `
            ...
            ${canBeConcluded ? `<div class="project-actions"><button type="button" class="btn-primary" id="conclude-project-btn">Concludi</button></div>` : ''}
        `;

        const concludeBtn = document.getElementById('conclude-project-btn');
        if (concludeBtn) {
            concludeBtn.addEventListener('click', async () => {
                clearMessages();

                const response = await fetch(`${ctx}/manager-home?action=concludeProject`, {
                    method: 'POST',
                    headers: {
                ...
                if (!response.ok) {
                    showError(payload.error || 'Unable to conclude project.');
                    return;
                }
