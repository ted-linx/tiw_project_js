import {clearMessages, showError} from './utils.js';
...
(() => {
    ...
    async function loadCollaborators() {
        try {
            const data = await getJSON(`${ctx}/manager-home?action=monitorCollaboratorsList`);
    ...
    async function selectCollaborator(username) {
        try {
            ...
            renderNav();
            const data = await getJSON(`${ctx}/manager-home?action=monitorCollaborator&username=${encodeURIComponent(username)}`);
            renderDetails(normalizeCollaboratorData(data));
        } catch (err) {
            showError("Error loading collaborator details");
        }
    }
    ...
    function renderProjectCard(project) {
        ...
        return `
            <div class="wp-card">
                ...
                ${wps.length ? `<div class="wp-list" style="padding: var(--space-4);">${wps.map(wp => renderWorkPackageCard(wp)).join('')}</div>` : `
                    <div class="empty-state empty-state-inline">
    ...
    function normalizeProject(project) {
        return {
            ...
            wps: (project.visibleWPs || project.workPackages || []).map((wp, index) => normalizeWorkPackage(wp, index, project.tasksByWp[String(wp.id)])),
        };
    }

    function normalizeWorkPackage(wp, index, tasks) {
        const normalized = {
        ...
        normalized.tasks.map((task, taskIndex) => normalizeTask(task, taskIndex, normalized));
